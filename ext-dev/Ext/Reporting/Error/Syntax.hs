...
module Ext.Reporting.Error.Syntax
  ( toReport
  , toSpaceReport
  )
  where
...
import Reporting.Error.Syntax hiding (toReport, toSpaceReport)
import Ext.Reporting.Render.Code
  ...
  case exposing of
    ...
    ExposingStart row col ->
      ...
      Report.Report "PROBLEM IN EXPOSING" region [] $
        Ext.Reporting.Render.Code.toSnippet source surroundings (Just region)
          (
            D.reflow $
