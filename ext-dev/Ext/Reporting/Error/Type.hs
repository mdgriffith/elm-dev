...
module Ext.Reporting.Error.Type
  ( toReport
  )
  where
...
import qualified Ext.Reporting.Render.Code
...
toPatternReport source localizer patternRegion category tipe expected =
  ...
  case expected of
    PNoExpectation expectedType ->
      Ext.Reporting.Render.Code.toSnippet source patternRegion Nothing $
    ...
    PFromContext region context expectedType ->
      Ext.Reporting.Render.Code.toSnippet source region (Just patternRegion) $
        case context of
    ...
    FromContext region context expectedType ->
      let
        ...
        custom maybeHighlight docPair =
          Report.Report "TYPE MISMATCH" exprRegion [] $
            Ext.Reporting.Render.Code.toSnippet source region maybeHighlight docPair
      in
      case context of
        ListEntry index ->
        ...
        CallArity maybeFuncName numGivenArgs ->
          ...
          Ext.Reporting.Render.Code.toSnippet source region (Just exprRegion) $
...
toInfiniteReport source localizer region name overallType =
  Report.Report "INFINITE TYPE" region [] $
    Ext.Reporting.Render.Code.toSnippet source region Nothing
      (
        D.reflow $
