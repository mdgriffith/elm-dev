...
module Ext.Reporting.Error.Canonicalize
  ( toReport
  )
  where
...
import Reporting.Error.Canonicalize hiding (toReport)
      ...
      case (unusedVars, unboundVars) of
        (unused:unuseds, []) ->
          ...
          Report.Report title aliasRegion [] $
            Ext.Reporting.Render.Code.toSnippet source aliasRegion subRegion
              (
                D.fillSep overview
              ,
                D.stack
        ...
        (_, _) ->
          ...
          Report.Report "TYPE VARIABLE PROBLEMS" aliasRegion [] $
            Ext.Reporting.Render.Code.toSnippet source aliasRegion Nothing
              (
                D.reflow $
...
unboundTypeVars source declRegion tipe typeName allVars (unboundVar, varRegion) unboundVars =
  ...
  Report.Report title declRegion [] $
    Ext.Reporting.Render.Code.toSnippet source declRegion subRegion
      (
        D.fillSep overview
      ,
        D.stack
