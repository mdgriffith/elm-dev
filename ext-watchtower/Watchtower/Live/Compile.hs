...
compileAll state@(Client.State _ mProjects _ _ _ _ _ _ _) = do
...
recompile state@(Client.State mClients mProjects _ _ _ _ _ _ _) allChangedFiles = do
...
toAffectedProject changedFiles projCache@(Client.ProjectCache proj@(Ext.Dev.Project.Project elmJsonRoot _ entrypoints _srcDirs _shortId) docsInfo _ _ _) =
...
recompileFile state (top, remain, projCache@(Client.ProjectCache proj@(Ext.Dev.Project.Project elmJsonRoot _ entrypoints _srcDirs _shortId) docsInfo _ mCompileResult _)) =
  do
    let (Client.State mClients _ _ _ _ _ _ _ _) = state
