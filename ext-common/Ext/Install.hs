...
installDependency :: Pkg.Name -> IO (Either DependencyManager.Error InstallResult)
installDependency package =
  do  maybeRoot <- Stuff.findRoot
      case maybeRoot of
        Nothing -> return (Left DependencyManager.NoOutline)
        Just root ->
          do  planned <- DependencyManager.planInstall root DependencyManager.Production [package]
              case planned of
